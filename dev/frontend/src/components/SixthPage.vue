<template>
    <section id="sixth_page_cont" class="page_cont">
        <div id="product_state_handler_cont">
            <font-awesome-icon :class="this.state_selected==='Shop' ?'state_icon_active' :'state_icon'" icon="fa-solid fa-shop" @click="()=>this.state_selected='Shop'"/>
        </div>
        ...
        <h3 class="el_counter">{{this.displayable_shop_els.length}} prodotti trovati</h3>

        <div v-if="this.state_selected==='Shop'" id="shop_cont">
            <div class="for_shop" v-for="(el, i) in this.displayable_shop_els" :key="'shop_'+i.toString()">
            <img class="for_shop_img" :src="el['image']"/>
            <h3>{{el["caption"] }}</h3>
            <p class="for_shop_type">{{el['type']}}</p>
            <div class="for_shop_footer">
                <div :class="saved_items.some(d=>isEqual_(d,el)) ?'heart_icon_active' :'heart_icon'">
...
export default{
    ...
    methods: {
        remove_html_el(i){
            this.$el.querySelectorAll('#cart_cont>*').item(i).remove()
        },
        onlySaved(el){
    ...
    watch: {
        type_selected: function () {
            if(this.type_selected==="tutti"){
                ...
                console.log("EL", this.$el.querySelector('#shop_cont'))
...
#sixth_page_cont >>> .dropdown-menu.show{
    width: 100%!important;
}
